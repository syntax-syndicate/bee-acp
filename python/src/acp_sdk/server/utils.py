...
async def watch_util_stop(run_data: RunData, store: Store[RunData]) -> AsyncGenerator[RunData]:
    async for data in run_data.watch(store):
        yield data
        if data.run.status == RunStatus.AWAITING:
            break


async def wait_util_stop(run_data: RunData, store: Store[RunData]) -> RunData:
    data = run_data
    async for latest_data in watch_util_stop(run_data, store):
        data = latest_data
    return data

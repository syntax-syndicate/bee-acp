...
async def main_agent(input: list[Message], context: Context) -> AsyncGenerator:
    session_storage[context.session].extend(input)

    llm = ChatModel.from_name("ollama:llama3.1:8b")
    agent = ReActAgent(
        llm=llm,
        tools=[
            HandoffTool("spanish_agent", context.session, session_storage),
            HandoffTool("english_agent", context.session, session_storage),
        ],
        templates={
            "system": lambda template: template.update(
                defaults={
